...
contract Cred is IssuerRole{
    ...
    function shareData(int256[128][] memory matrix, uint round) public onlyIssuer returns (uint, int, int) {
        int val = 0;
        int score = 0;
        int reward = 0;
        int totalScore = 0;
    ...
    function cosineSim(int[128][] memory newMatrix, uint round) internal view returns (int) {
        ...
        int magA = vectorMag(A_flat);
        int magB = vectorMag(B_flat);
        if (magA==0 || magB==0) {
            return 0;
        }
        uint key = uint(magB);
        ...
        int256 dot = dotProduct(A_flat, B_flat);
        ...
        return int(dot * 100 / (magA * magB));
    }

    function checkGrad(int[128][] memory grad) internal pure returns (bool) {
        
        int[] memory flat = flatten(grad);

        int mag = vectorMag(flat);
